<?php
class TaskApplicationsData extends CI_Model {
    private $table = "tasks_applications";
    private $pk = "id";
    private $date = null;
    ...
    function getapplicants($key,$value,$key2=null, $value2=null){
        if (($key2) && ($value2)){
            return  $this->db->query("SELECT  $this->table.*, members.username, members.firstname, members.lastname, members.profile_image FROM $this->table INNER JOIN members on $this->table.userid = members.id where `$key` = '".$value."' AND `$key2` = '".$value2."'");
        }else {
			return  $this->db->query("SELECT  $this->table.*, members.username, members.firstname, members.lastname, members.profile_image FROM $this->table INNER JOIN members on $this->table.userid = members.id where `$key` = '".$value."'");
        }
    }
