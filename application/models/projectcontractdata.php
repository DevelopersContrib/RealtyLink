<?php
class ProjectContractData extends CI_Model {
    private $table = "project_contracts";
    private $pk = "id";
    private $date = null;
    ...
    function hascontract($project_id,$ctype){
        $has = false;
        $query = $this->db->query("SELECT COUNT(*) AS total FROM project_contracts WHERE project_id='$project_id' AND `contract_type` = '$ctype'");
        if ($query->num_rows() > 0){
            foreach ($query->result() as $row)
            {
                $total =  $row->total;
	...
	function getcontract($project_id,$ctype){
        $row = array();
        $query = $this->db->query("SELECT * FROM project_contracts WHERE project_id='$project_id' AND `contract_type` = '$ctype'");
        if ($query->num_rows() > 0){
			$row = $query->row();
        }
        return $has;
