					...
					<div class="card newsfeed-card mb-3 ">
						<div class="card-body">
							<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
								...
								<a class="nav-link <?//=$business?>" id="v-pills-license-tab" data-toggle="pill" href="#v-pills-license" role="tab"> <i class="fas fa-certificate mr-2" aria-hidden="true"></i> License </a>
								<a class="nav-link <?//=$business?>" id="v-pills-bond-tab" data-toggle="pill" href="#v-pills-bond" role="tab"> <i class="fas fa-file-contract mr-2" aria-hidden="true"></i> Bond </a>
								<a class="nav-link <?//=$business?>" id="v-pills-socials-tab" data-toggle="pill" href="#v-pills-socials" role="tab"> <i class="fas fa-rocket mr-2" aria-hidden="true"></i> Socials </a>
								<a class="nav-link" id="v-pills-home-tab" data-toggle="pill" href="#v-pills-notification" role="tab"> <i class="fas fa-copy mr-2" aria-hidden="true"></i> Notification Settings </a>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="card newsfeed-card mb-3 ">
						<div class="card-body">
							<div class="tab-content" id="v-pills-tabContent">
														...
														<div class="col">
															<select name="owner_country" id="owner_country" class="form-control custom-select">
																...
																<?php if ($countries->num_rows() > 0):?>
																	...
																	<option <?=$row->country_id==$member['country_id']?'SELECTED':''?> value="<?php echo $row->country_id?>"><?php echo $row->name.' (+'.$row->phone_code.')'?></option>
																	<?php endforeach;?>
																<?php endif?>
															</select>
														</div>
								...
								<div class="tab-pane fade <?//=$security_show?>" id="v-pills-license" role="tabpanel">
									<?php $this->load->view('dashboard/license_form',['licenseDetails'=>$licenseDetails]) ?>
								</div>
								<div class="tab-pane fade <?//=$security_show?>" id="v-pills-bond" role="tabpanel">
									<?php $this->load->view('dashboard/bond_form',['bondDetails'=>$bondDetails]);?>
								</div>
								<div class="tab-pane fade <?//=$security_show?>" id="v-pills-socials" role="tabpanel">
									<?php $this->load->view('dashboard/socials_form',['socials'=>$memberSocials]);?>
								</div>
								<div class="tab-pane fade" id="v-pills-notification" role="tabpanel">
								    <?php $this->load->view('dashboard/settings_notification');?>
								</div>
							</div>
						</div>
					</div>
...
$( document ).ready(function() {
	$('#request_temp_pass').click(function(){
		...
		$.post('/settings/requestpassword',function(data){
			if(data.status){
	...
	$('#profile_save').click(function(){
		...
		$(this).html('<span class="loading-icon">&nbsp;<i class="fa fa-spinner fa-spin"></i></span>');
		$.post('/settings/ajaxSaveSettings',{
	...
	$('#password_save').click(function(){
		...
		if(newpass.length < 5) {
			...
			$.post('/settings/checkoldpass',{
				oldpass:oldpass
			},function(data){
				if(data.status){
					$.post('/settings/savepassword',{
						newpass:newpass,
						oldpass:oldpass
					},function(data){
						$('#password_save').html('<i class="fas fa-check"></i> Change Password');
